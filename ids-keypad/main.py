...
def renderMainMenu():
    ...
    title = "Menu"
    pageCount = str(menuPage + 1) + "/" + str(len(menuOptions))
    ...
    renderCustomFont(roboto16, title, 5, 0)
    renderCustomFont(roboto12, pageCount, 60, 5)
...
def handleMenu(direction):
    ...
    menuLength = len(menuOptions)
    if direction == "DOWN":
        if menuMode and not subMenuMode:
            menuPage = (menuPage + 1) % len(menuOptions)

            if menuPage == 0:
                selectedPosition = 0
                shownOptions = [0, 1, 2]
            else:
                nextPosition = (selectedPosition + 1) % 3
                selectedPosition = nextPosition if nextPosition != 0 else selectedPosition
                if menuPage not in shownOptions:
                    shownOptions = [shownOptions[0] + 1, shownOptions[1] + 1, shownOptions[2] + 1]
    ...
    if direction == "UP":
        if menuMode and not subMenuMode:
            # iterate menu page and reset
            menuPage = (menuPage - 1) % menuLength

            if menuPage == menuLength - 1:
                selectedPosition = 2
                shownOptions = [menuLength - 3, menuLength - 2, menuLength - 1]
            else:
                nextPosition = (selectedPosition - 1) % 3
                selectedPosition = nextPosition if selectedPosition != 0 else selectedPosition
                if menuPage not in shownOptions:
                    shownOptions = [shownOptions[0] - 1, shownOptions[1] - 1, shownOptions[2] - 1]
